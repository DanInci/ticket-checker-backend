...
/**
  ...
  *
  */
private[ticket]final case class TicketOrganizerImpl[F[_]](private val ticketAlgebra: TicketAlgebra[F])(implicit F: Sync[F]) extends TicketOrganizer[F] {
  ...
  override def createTicket(orgId: OrganizationID, definition: TicketDefinition)(implicit ctx: UserAuthCtx): F[Ticket] =
    for {
      ticket <- ticketAlgebra.create(orgId, definition)
    } yield ticket
  ...
  override def setTicketValidationStatus(orgId: OrganizationID, ticketId: TicketID, isValidated: IsValidated)(implicit ctx: UserAuthCtx): F[Ticket] =
    for {
      ticket <- ticketAlgebra.setValidationStatusById(orgId, ticketId, isValidated)
    } yield ticket
